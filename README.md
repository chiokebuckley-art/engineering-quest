...
**Calculus Prep** (Arcade → Calculus Prep; `src/engine/questions/precalc.ts`) is the algebra directly beneath calculus,
chemistry and engineering, twelve kinds with six lessons: *fractions* — add and subtract by finding the common
denominator (drawn as fraction bars re-cut into matching pieces), multiply straight across, divide by keep-change-flip
(`1/2 + 1/3 = 5/6`); *negative numbers* — minus a negative on a number line, sign rules for × and ÷
(`−3 − (−8) = 5`); *exponents* — product, quotient and power rules, zero and negative exponents (`x³x² = x⁵`);
*roots* — square and cube roots, roots of powers, simplifying `√50 = 5√2`, estimating; *rearranging equations* — pick
the correct rearrangement of a real formula (`PV = nRT ⇒ T = PV/(nR)`, Ohm's law, F = ma, density, power, v = u + at,
kinetic energy, y = mx + b) from four, taught as "undo what is done to the letter, on both sides"; *using a formula*
— substitute and carry the unit; *scientific notation* — writing, reading, multiplying and dividing on the
place-value chart; *unit conversions* — prefixes, areas, volumes and `72 km/h = 20 m/s` drawn as cancelling
fractions; *functions* — f(4), f(−3), f(g(2)) and running the machine backwards; *lines and graphs* — slope,
intercept and reading a value off a drawn grid; *trigonometry* — SOH CAH TOA ratios on a labelled triangle, finding a
side, finding an angle with tan⁻¹, special angles. Answers can be typed as fractions (`5/6`, any equivalent is
accepted) or signed numbers: the keypad gains `/` and `−` keys when a question allows them (`Question.allowFraction`,
`allowNegative`), and `answerText` shows the answer the way it is written. Visuals `fracbar|grid|trirat`; skills
`precalc`, `precalc.frac.add|frac.mul|neg|exp|roots|rearrange|evaluate|scinot|units|func|graph|trig`.
