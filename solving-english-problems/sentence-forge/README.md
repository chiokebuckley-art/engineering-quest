...
The canonical source is kept in Command Center at `solving-english-problems/sentence-forge/`, as one of the three worlds of **Solving English Problems** (see `../README.md`). Relative assets and the service-worker scope keep it usable at any path, with its own save key; the previous `engineering-quest/public/sentence-forge/` location now redirects here. Every view is deep-linkable (`#factory/<district>`, `#arcade/<mode>`, `#learn/<district>`, `#catalog`, `#repair`, `#progress`, `#adventure`, `#reading`), and `../hub-bar.js` adds the shared home button. No existing math game state or screen was modified.
