...
export function validatePack(p){
 ...
 for(const c of p.cards){if(!text(c.id,150)||cards.has(c.id)||!pages.has(c.pageId)||!text(c.term,200)||(c.sourceMode!=='scan'&&(!text(c.meaning,2000)||!text(c.example,2000)||!text(c.notice,2000)))||!ROADMAP.includes(c.chapter))throw Error('A meaning card in this pack is invalid. Nothing was imported.');cards.add(c.id);if(c.image&&!/^data:image\/(jpeg|png);base64,[A-Za-z0-9+/=]+$/.test(c.image))throw Error('Invalid item image.');if(c.sourceDefinition&&!text(c.sourceDefinition,4000))throw Error('Invalid source definition.');}
 for(const c of p.cards){const page=p.pages.find(x=>x.id===c.pageId);for(const rect of [c.crop,c.definitionCrop,c.labelBox].filter(Boolean)){if(!Array.isArray(rect)||rect.length!==4||rect.some(v=>!Number.isFinite(v)||v<0)||rect[2]<=rect[0]||rect[3]<=rect[1]||rect[2]>(page.layout?.[0]||3000)||rect[3]>(page.layout?.[1]||4000))throw Error('Invalid source crop.');}}
